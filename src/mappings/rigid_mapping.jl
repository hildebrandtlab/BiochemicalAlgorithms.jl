...
function compute_rmsd(f::AbstractAtomBijection{T}) where {T<:Real}
    r_BA = Vector{Vector3{Angstrom{T}}}(undef, size(f.atoms_A, 1))
...
function compute_rmsd_minimizer(f::AbstractAtomBijection{T}) where {T<:Real}
    r_A = Vector{Vector3{Angstrom{T}}}(undef, size(f.atoms_A, 1))
    DataFramesMeta.@with f.atoms_A r_A .= :r
    mean_A = mean(r_A)

    r_B = Vector{Vector3{Angstrom{T}}}(undef, size(f.atoms_B, 1))
