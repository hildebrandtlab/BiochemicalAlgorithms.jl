...
function place_peptide_bond_h_!(frag::Fragment{T}) where T
    ...
    if is_amino_acid(frag) && !is_n_terminal(frag)
        ...
        h_atom = Atom(
            frag, 
            maximum(atoms_df(frag.sys).number)+1, # does this make sense?
            Elements.H,
            "H",
            "",
...
function _add_hydrogen!(atom::Atom{T}, r::Vector3{T}, atom_nr::Int) where {T<:Real}
    ...
    h_atom = Atom(
        ...
        maximum(atoms_df(atom.sys).number)+1, # does this make sense?
        Elements.H,
    ...
    Bond(atom.sys, atom.idx, h_atom.idx, BondOrder.Single, Properties())
end
...
function _handle_atom!(atom::Atom{T}, mmff_params::MMFF94Parameters{T}, is_ring_atom::Bool, atom_nr::Int) where {T<:Real}
    ...
    if nbonds(atom) == 1 && bonds(atom)[1].order == BondOrder.Aromatic
        return 0, atom_nr
    end
    ...
    if (h_to_add == 1 && nr_bonds == 1 && sum_bond_orders > 2)
        partner_atom = get_partner(bonds(atom)[1], atom)
    ...
    if (con == 2)
        if (h_to_add == 2)
            ...
            # h_to_add == 1
            bv = atom.r - get_partner(bonds(atom)[1], atom).r
            axis = _get_normal(bv)
    ...
    if is_ring_atom
        ...
        if (con == 3 || (con == 4 && atom.formal_charge == 1))
            if (h_to_add == 1)
                _add_hydrogen!(aom, atom.r + v3, atom_nr);
        ...
        if any(map(b -> b.order > BondOrder.Single, bonds(a)))
            bv = normalize(get_partner(bonds(atom)[1], atom).r - atom.r)
