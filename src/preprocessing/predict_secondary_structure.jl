...
const DSSP_DEBUG = false

function _insert_turn!(target_n_turn::String, turn_char::Char, position::Int)
	# first sign
    target_n_turn[position] = (target_n_turn[position] ∈ ['<', 'X']) ? 'X' : '>'
	...
	# last position 
	target_n_turn[position+turn] = (target_n_turn[position+turn] ∈ [ '>' ,'X']) ? 'X' : '<'
end
...
function _prepare_hbonds(ac::AbstractAtomContainer)
...
function postprocess_dssp_!(ac::AbstractAtomContainer, summary::Vector{Char})
    last_ss = nothing
    last_ss_type = 'x'
    last_chain_idx = -1
    ss_number = 1
...
function create_new_ss_(chain::Chain, number::Int, type::Char)
    ss = nothing
