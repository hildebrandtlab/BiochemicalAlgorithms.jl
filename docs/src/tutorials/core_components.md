...
println("Bond index: ", bond.idx, ", between atoms ", a1.idx, " and ", a2.idx,
    ", with order: ", bond.order)
...
```
    ...
    Bond index: 30, between atoms 4 and 5, with order: Single
...
We have used `get_partners` above to retrieve both atoms associated with a bond. Similarly, `get_partner` can be used to retrieve the second atom if one atom of a bond is already known:
