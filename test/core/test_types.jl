...
@testitem "_RowProjectionVector" begin
    ...
    for T in [Int, Float32, Float64]
        ...
        # invalid row bounds
        @test_throws BoundsError _RowProjectionVector(v, [0], Dict(0 => 0))[1]
        @test_throws BoundsError _RowProjectionVector(v, [4], Dict(4 => 4))[1]

        # invalid mapping
        @test_throws KeyError _RowProjectionVector(v, [1], Dict(0 => 0))[1]

        # row subset
        u = _RowProjectionVector(v, [3, 1], Dict(3 => 3, 1 => 1))
        ...
        u = _RowProjectionVector(v, [3, 1], Dict(3 => 1, 1 => 3))
        @test u[1] == v[1]
        u[1] = T(11)
        @test u[1] == v[1] == 11
        @test u[2] == v[3]
        ...
        # row bag
        u = _RowProjectionVector(v, [2, 2], Dict(2 => 2))
        @test size(u) == (2,)
        @test length(u) == 2
        @test_throws BoundsError u[0]
        @test_throws BoundsError u[0] = T(41)
